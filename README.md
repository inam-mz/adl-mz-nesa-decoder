...
ADL Decoder for Mozambique NESA Stations
...
The ADL MZ Nesa Decoder is a module intended to be installed in an [ADL](https://github.com/wmo-raf/adl)
...
### Install ADL MZ Nesa Decoder

The `dev.Dockerfile` file uses the `adl` image as a base image. The `ADL MZ Nesa Decoder` is
...
```bash
git clone https://github.com/wmo-raf/adl-mz-nesa-decoder.git
